...
# Función para crear los botones de búsqueda en Google
def boton_consulta_directa(diagnostico_txt):
    ...
    st.markdown(
        ...
        f'padding:6px 12px; text-align:center; text-decoration:none; '
        f'display:inline-block; font-size:14px; margin:2px 0px; cursor:pointer; '
        f'border-radius:6px;">🔍 Fotos y Tratamiento</button></a>',
        unsafe_allow_html=True
    )

# Estructura visual en 3 columnas en paralelo (Evita la linealidad)
...
with col1:
    ...
    if st.checkbox("Tiene agujeros grandes e irregulares con rastro plateado"):
        diagnosticos_detectados["BABOSAS / CARACOLES / LIMACOS"] = "Limacos babosas"
    ...
    if st.checkbox("Hay agujeros con el borde marrón"):
        diagnosticos_detectados["CÁPSIDO VERDE COMÚN"] = "Capsido verde comun"
    if st.checkbox("Hay pequeños insectos y puestas de huevos diminutos (hojas cloróticas)"):
    ...
    if st.checkbox("Las hojas jaspeadas se vuelven marrones"):
        diagnosticos_detectados["FALTA DE LUZ NATURAL (Zona de umbría)"] = "Falta de luz natural plantas"
    if st.checkbox("La planta ya no florece"):
        diagnosticos_detectados["CARENCIA DE POTASIO (K)"] = "Falta de potasio plantas"
...
with col2:
    ...
    if st.checkbox("Los tallos/hojas están dañados/quemados pero la planta sobrevive"):
        diagnosticos_detectados["QUEMADURA DE LAS HOJAS"] = "Quemadura de las hojas plantas"
...
with col3:
    ...
    if st.checkbox("Las hojas se vuelven marrones sólo por la punta"):
        diagnosticos_detectados["EXCESO DE ABONO / LIMITACIÓN DE ESPACIO"] = "Exceso de abono puntas marrones"
    if st.checkbox("Las hojas se vuelven marrones por los bordes"):
        diagnosticos_detectados["QUEMADURA POR EL VIENTO"] = "Quemadura por viento plantas"
    if st.checkbox("Las hojas son pálidas y demasiado pequeñas (generalizado)"):
        diagnosticos_detectados["TIERRA ESTÉRIL / CARENCIA DE NITRÓGENO (N)"] = "Falta de nitrogeno plantas"
    ...
    if st.checkbox("Hojas secas, crujientes o marchitamiento por tierra seca"):
        diagnosticos_detectados["AUSENCIA DE RIEGO"] = "Falta de riego plantas"
...
if diagnosticos_detectados:
    ...
    # Mostramos cada problema detectado con su propio botón independiente
    for nombre_problema, termino_busqueda in diagnosticos_detectados.items():
        col_diag, col_bot = st.columns([2, 1])
        with col_diag:
